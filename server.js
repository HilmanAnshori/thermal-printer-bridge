...
const loadConfig = () => {
    ...
    return {
        ...
        panelPort: parseInt(
            envVal("PANEL_PORT", fileConfig.panelPort || 3008),
            10
        ),
        printer: {
            driver: (
                envVal(
                    "PRINTER_DRIVER",
                    fileConfig.printer?.driver || "network"
                ) || ""
            )
                .toLowerCase()
            ...
            bluetoothChannel: parseInt(
                envVal(
                    "PRINTER_BT_CHANNEL",
                    fileConfig.printer?.bluetoothChannel || 1
                ),
                10
            ),
            usb: {
...
const buildNextConfig = (payload = {}) => {
    ...
    const nextDriver = (
        printerPayload.driver ?? config.printer.driver ?? "network"
    )
        .toLowerCase()
        .replace("rfcomm", "bluetooth");
...
const createDevice = () => {
    ...
    if (driver === "usb") {
        ...
        if (isNaN(vidInt) || isNaN(pidInt)) {
            throw new Error(
                `VendorID/ProductID USB tidak valid: ${vid}/${pid}`
            );
        }
    ...
    if (driver === "bluetooth") {
        ...
        return new escpos.Serial("/dev/rfcomm0", { autoOpen: false, lock: false, baudRate: 9600 });
    }
            ...
            const printLogic = (logo) => {
                try {
                    ...
                    if (logo) {
                        ...
                        console.log("[Bridge] logo.jpg tidak ditemukan, lanjut cetak teks.");
                    }
                    
                    console.log("[Bridge] Mencetak struk...");
                    // console.log("[Bridge] Receipt payload:", text.replace(/\n/g, " | "));
                    printer.text(text).cut().close((closeErr) => {
...
wss.on("connection", (ws, req) => {
    ...
    ws.on("message", async (raw) => {
        ...
        if (data.type === "status") {
            ...
            broadcastLog(
                `[Status] connected=${connection.connected} · pending=${stats.pending ?? 0}`
            );
            sendWs(ws, {
                type: "status",
                stats,
        ...
        if (data.type === "scan-usb") {
            ...
            exec(lsusbCmd, { timeout: 5000 }, (error, stdout) => {
                ...
                broadcastLog(
                    `[Scan USB] ${devices.length} device(s) discovered.`
                );
                sendWs(ws, {
...
app.post("/panel/api/config", (req, res) => {
    try {
        ...
        persistConfig(nextConfig);
        res.json({ success: true, message: "Konfigurasi disimpan. Bridge restart..." });
        scheduleRestart();
    } catch (error) {
